...
const cliGO = async () => {
  let puzInput = getPuzzleInput();
  let total = 0;
  for await (const line of puzInput) {
    let rowVal = getDigits(line);
    total += rowVal;
    console.log(total);
  }
  return total;
...
const getDigits = (line: string): number => {
  let searchArr = ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"];
  let start = 9999999;
  let end = -1;
  let startNum = "";
  let endNum = "";
  for (const num of searchArr) {
